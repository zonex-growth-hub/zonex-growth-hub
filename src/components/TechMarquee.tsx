...
export function TechMarquee() {
  ...
  return (
    <section className="py-8 relative overflow-hidden select-none bg-[#030305]">
      <div className="max-w-7xl mx-auto px-4 mb-4">
        <p className="text-center text-xs uppercase tracking-widest text-slate-400 font-bold">
          High-Performance Stack &amp; Media Infrastructure
        </p>
      </div>
      <div className="relative">
        {/* Obsidian gradient fade edges */}
        ...
        <div className="flex animate-marquee gap-6 w-max will-change-transform">
          {items.map((tech, i) => (
            <div
              key={`${tech.name}-${i}`}
              onClick={playClick}
              className="group/flip w-[190px] h-[72px] perspective-1000 cursor-pointer shrink-0"
            >
              <div className="relative w-full h-full transition-transform duration-500 transform-style-3d group-hover/flip:rotate-y-180">
                
                {/* Front Side */}
                <div className="absolute inset-0 bg-[#0B0B10]/80 backdrop-blur-xl border border-white/[0.08] shadow-lg rounded-2xl flex items-center gap-3 px-5 backface-hidden">
                  <span className="w-2.5 h-2.5 rounded-full bg-[#8A63F8] shadow-[0_0_10px_#8A63F8] animate-pulse" />
                ...
                {/* Back Side */}
                <div className="absolute inset-0 bg-gradient-to-r from-purple-900 to-indigo-900 rounded-2xl flex flex-col items-center justify-center text-white rotate-y-180 backface-hidden px-3 shadow-xl border border-purple-500/40">
                  <p className="text-[9px] uppercase font-bold tracking-widest text-purple-200">{tech.label}</p>
                  <p className="text-xs font-black mt-0.5 text-cyan-300">{isINR ? tech.outcomeINR : tech.outcomeUSD}</p>
                </div>
